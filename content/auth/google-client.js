...
class GoogleClient {
  constructor() {
    ...
    this.USER_DATA_RANGE = "A2:E"; // Full user data range (A=Name, B=Email, C=Picture, D=Timestamp, E=Status)
  ...
  async appendUserData(userData) {
    try {
      ...
      const response = await this.makeRequest(`/values/${this.USERS_SHEET_NAME}!${this.USER_DATA_RANGE}:append?valueInputOption=RAW`, {
        method: 'POST',
        body: JSON.stringify({
          values: values
        })
      ...
      console.log('aiFiverr Google Client: User data appended successfully');
      return response;
  ...
  async updateUserData(userData, rowIndex) {
    try {
      const range = `${this.USERS_SHEET_NAME}!A${rowIndex}:E${rowIndex}`;
      ...
      console.log('aiFiverr Google Client: User data updated successfully');
      return response;
